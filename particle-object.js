...
export function createParticleObject(elements, options = {}) {
  ...
  function onPointerMove(event) {
    ...
    if (isPointerDown && now - lastDisperseTime > 65) {
      lastDisperseTime = now;
      disperse(event.clientX, event.clientY, 0.6);
    }
  }
  ...
  function onPointerDown(event) {
    ...
    disperse(event.clientX, event.clientY, 1.35);
  }
  ...
  let disperseEnergy = 0.0;
  let lastDisperseCall = 0;

  function disperse(clientX, clientY, strength = 1.0) {
    if (!points || !homes || !velocities || particleCount === 0) return;
    const now = performance.now();
    if (now - lastDisperseCall < 20) return;
    lastDisperseCall = now;

    const rect = canvas.getBoundingClientRect();
    const px = clientX !== undefined ? clientX - rect.left : canvas.clientWidth * 0.5;
    const py = clientY !== undefined ? clientY - rect.top : canvas.clientHeight * 0.5;
    ...
    const v = velocities;
    const baseBlast = 26.0 * strength;
    const swirlFactor = 3.2;
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      // Distance falloff from touch ray
      const falloff = 1.0 / (1.0 + dist * 1.3);

      // Whirlwind vortex spin (cross product with ray direction)
      const tx = dy * rz - dz * ry;
      const ty = dz * rx - dx * rz;
      const tz = dx * ry - dy * rx;

      // Feather flutter randomness
      const randX = (Math.random() - 0.5) * 8.0;
      const randY = (Math.random() - 0.5) * 8.0;
      const randZ = (Math.random() - 0.5) * 8.0;

      const power = baseBlast * falloff;
      v[ix] += (rx * 1.5 + tx * swirlFactor + randX) * power;
      v[iy] += (ry * 1.5 + ty * swirlFactor + randY) * power;
      v[iz] += (rz * 1.5 + tz * swirlFactor + randZ) * power;
    }

    // Peak dispersal energy that decays smoothly in simulate()
    disperseEnergy = Math.min(1.0, disperseEnergy + 0.95 * strength);
    material.uniforms.uTint.value.setRGB(1.0, 0.08, 0.22);
  }
  ...
  function simulate(delta) {
    ...
    // Dispersal energy decays over ~1.8-2.0 seconds
    if (disperseEnergy > 0) {
      disperseEnergy = Math.max(0, disperseEnergy - delta * 0.52);
      // Crimson Sharingan chakra flare smoothly tracks dispersal energy
      material.uniforms.uUseTint.value = Math.max(config.color ? 1 : 0, disperseEnergy * 0.92);
    }

    const baseStiffness = 60 * Math.max(config.spring, 0.05);
    // Relax spring tension by up to 88% while dispersed so particles fly outward in a majestic cyclone
    const stiffness = baseStiffness * (1.0 - disperseEnergy * 0.88);
    const dampingRate = (3 + 12 * Math.min(Math.max(config.damping, 0), 1)) * (1.0 - disperseEnergy * 0.35);
    ...
    const activeDisperse = disperseEnergy > 0.01;
    const vortexStrength = disperseEnergy * 11.0 * delta;
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      if (activeDisperse) {
        // Swirling crow whirlwind vortex around Z-axis + organic flutter
        const px = p[ix];
        const py = p[iy];
        const r = Math.hypot(px, py) + 0.12;
        vx += (-py / r) * vortexStrength + Math.sin(elapsed * 4.0 + i * 0.2) * (vortexStrength * 0.35);
        vy += ( px / r) * vortexStrength + Math.cos(elapsed * 4.0 + i * 0.2) * (vortexStrength * 0.35);
        vz += Math.sin(elapsed * 3.0 + i * 0.3) * (vortexStrength * 0.45);
      }
